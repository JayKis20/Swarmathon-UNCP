...
//    centerLocationMap.x = 0;
//    centerLocationMap.y = 0;
    ...
    if (currentMode == 2 || currentMode == 3) {
        ...
        if (!init) {
            if (timerTimeElapsed > startDelayInSeconds) {
                ...
                //set initial random heading
                // Nathan moved it here from main()
//                goalLocation.theta = rng->uniformReal(0, 2 * M_PI);
//                goalLocation = searchController.search(currentLocationMap);
        ...
        case STATE_MACHINE_TRANSFORM: {
            ...
            if (targetCollected && !avoidingObstacle) {
                ...
                // Nathan changed centerLocation to centerLocationMap
        ...
        case STATE_MACHINE_PICKUP: {
            ...
            if (targetDetected && !targetCollected) {
                ...
                if (result.pickedUp) {
                    ...
                    goalLocation.x = centerLocation.x = 0;
...
void targetHandler(const apriltags_ros::AprilTagDetectionArray::ConstPtr& message) {
    ...
    if (message->detections.size() > 0 && !reachedCollectionPoint) {
        ...
        if (centerSeen && !targetCollected) {
            ...
            float centeringTurn = 0.17; //radians
            stateMachineState = STATE_MACHINE_TRANSFORM;
...
void obstacleHandler(const std_msgs::UInt8::ConstPtr& message) {
    if ((!targetDetected || targetCollected) && (message->data > 0)) {
        ...
        if (message->data == 1) {
            ...
            goalLocation.theta = currentLocation.theta + 0.78;
        }
...
void mapAverage() {
    ...
    if (init) {
        ...
        centerLocation.x = mapPose.pose.position.x;
        centerLocation.y = mapPose.pose.position.y;
